...
{
    public bool IsCombineSubMesh = true;
    ...
    public void OnDrawGizmos()
    {
        ...
        {
            if (((Selection.gameObjects[i] == this.gameObject) && m_meshSimplifyRoot == null) || ((m_meshSimplifyRoot != null) && (Selection.gameObjects[i] == m_meshSimplifyRoot.gameObject)))
            {
                bDrawVertices = true;
            }
        }
        ...
        for (int nVertex = 0; nVertex < aVerticesWorld.Length; nVertex++)
        {
            for (int nSphere = 0; nSphere < aRelevanceSpheres.Length; nSphere++)
            {
                if (aRelevanceSpheres[nSphere].m_bExpanded)
                {
                    ...
                    {
                        Gizmos.DrawCube(aVerticesWorld[nVertex], Vector3.one * HandleUtility.GetHandleSize(aVerticesWorld[nVertex]) * 0.05f);
                        break;
                    }
                }
            }
    ...
    public void ComputeMesh(bool bRecurseIntoChildren, Simplifier.ProgressDelegate progress = null)
    {
        ...
        if (IsCombineSubMesh && m_simplifiedMesh.subMeshCount > 1)
        {
            var mesh = m_simplifiedMesh;
            var combines = new CombineInstance[mesh.subMeshCount];
            ...
            m_simplifiedMesh = new Mesh() { name = mesh.name };
            m_simplifiedMesh.CombineMeshes(combines, true, true);
            m_simplifiedMesh.bindposes = mesh.bindposes;
            m_simplifiedMesh.boneWeights = mesh.boneWeights;
        }
    }
    ...
    private static Mesh CreateNewEmptyMesh(MeshSimplify meshSimplify)
    {
        ...
        Mesh meshOut = Instantiate(meshSimplify.m_originalMesh);
        meshOut.Clear();
        return meshOut;
    }
    ...
    private static void DisablePrefabUsageRecursive(MeshSimplify root, GameObject gameObject, bool bRecurseIntoChildren)
    {
        ...
        if (meshSimplify != null)
        {
            if (IsRootOrBelongsToTree(meshSimplify, root))
            {
                if (meshSimplify.m_simplifiedMesh)
                {
                    if (AssetDatabase.IsMainAsset(meshSimplify.m_simplifiedMesh) || AssetDatabase.IsSubAsset(meshSimplify.m_simplifiedMesh))
                    {
