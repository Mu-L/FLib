...
public class MeshSimplifyEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUI.BeginChangeCheck();
        meshSimplify!.IsCombineSubMesh = EditorGUILayout.Toggle("Is Combine SubMesh", meshSimplify.IsCombineSubMesh);
        if (EditorGUI.EndChangeCheck())
            EditorUtility.SetDirty(meshSimplify);
        ...
        if (m_bRemoveFromTree && Event.current.type == EventType.Repaint)
        {
            ...
            {
                DestroyImmediate(meshSimplify);
            }
            else
            {
                Destroy(meshSimplify);
            }
            ...
            foreach (Object targetObject in targets)
            {
                ...
                if (meshSimplify.m_meshSimplifyRoot == null && meshSimplify.m_bEnablePrefabUsage)
                {
                    ...
                    if (string.IsNullOrEmpty(strMeshAssetPath))
                    {
                        ...
                        var path = Path.GetDirectoryName(PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(gameObject));

                        strMeshAssetPath = EditorUtility.SaveFilePanelInProject("Save mesh asset(s)", $"{gameObject.name}", "mesh", "", path);
                    ...
                    SaveMeshAssetsRecursive(gameObject, gameObject, strMeshAssetPath, true, File.Exists(strMeshAssetPath), ref nCounter);
                }
            }
        }
        catch (System.Exception e)
    ...
    bool SaveMeshAssetsRecursive(GameObject root, GameObject gameObject, string strFile, bool bRecurseIntoChildren, bool bAssetAlreadyCreated, ref int nProgressElementsCounter)
    {
        ...
        if (meshSimplify != null && meshSimplify.HasData() && (meshSimplify.m_meshSimplifyRoot == null || meshSimplify.m_meshSimplifyRoot.gameObject == root))
        {
            ...
            if (meshSimplify.m_simplifiedMesh != null && MeshSimplify.HasValidMeshData(meshSimplify.gameObject))
            {
                ...
                if (bAssetAlreadyCreated == false && AssetDatabase.Contains(meshSimplify.m_simplifiedMesh) == false)
                {
                    //Debug.Log("Creating asset " + meshSimplify.m_simplifiedMesh.name);

                    AssetDatabase.CreateAsset(meshSimplify.m_simplifiedMesh, strFile);
                    bAssetAlreadyCreated = true;
                }
                else
                {
                    if (AssetDatabase.Contains(meshSimplify.m_simplifiedMesh) == false)
                    {
                        //Debug.Log("Adding asset " + meshSimplify.m_simplifiedMesh.name);
                        AssetDatabase.RemoveObjectFromAsset(AssetDatabase.LoadMainAssetAtPath(strFile));
                        AssetDatabase.AddObjectToAsset(meshSimplify.m_simplifiedMesh, strFile);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(meshSimplify.m_simplifiedMesh));
                    }
                }
