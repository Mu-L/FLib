...
{
    public class DynamicPackingBinaryTree : DynamicPacking
    {
        private class Node
        {
        ...
        private readonly Dictionary<uint, Node> _nodeMap = new(32);

        protected DynamicPackingBinaryTree()
        {
            _rootNode = Node.Rent();
        }

        public DynamicPackingBinaryTree(Vector2Int size, int padding) : base(size, padding)
        {
            _rootNode = Node.Rent();
        ...
        protected override bool OnAddImage(uint imageId, int width, int height, out Vector2Int pos)
        {
        ...
        protected override bool OnFreeImage(uint imageId)
        {
