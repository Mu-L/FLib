...
namespace Kamgam.UIPreview
{
    ...
    public class UIPreview : ObjectPreview
    {
        ...
        static Texture2D ErrorTexture = new(2, 2);

        /// <summary>
        /// Cache with one texture per prefab.
        /// </summary>
        static Dictionary<Object, Texture2D> previews = new();
        ...
        protected Dictionary<int, bool> _shouldBeShownLookupTabe = new();
        ...
        protected bool shouldBeShown()
        {
            ...
            if (_shouldBeShownLookupTabe.TryGetValue(id, out var shown))
            {
                return shown;
            }
        ...
        protected void createPreviewTextureForTarget(Object t)
        {
            var settings = UIPreviewSettings.GetOrCreateSettings();
            ...
            try
            {
                ...
                if (previews.ContainsKey(t))
                    previews[t] = CreateAssetPreview(t);
                else
                    previews.Add(t, CreateAssetPreview(t));
            }
        ...
        static bool reflectionCacheBuilt;
        ...
        public static Texture2D CreateAssetPreview(Object target)
        {
            ...
            // cam
            var cameraObj = EditorUtility.CreateGameObjectWithHideFlags("camera", HideFlags.DontSave);
            ...
            // canvas
            var canvasObj = EditorUtility.CreateGameObjectWithHideFlags("canvas", HideFlags.DontSave, typeof(Canvas));
            SceneManager.MoveGameObjectToScene(canvasObj, previewScene);
            ...
            var obj = Object.Instantiate((GameObject)target, canvasObj.transform, false);
            ...
            for (var i = rectTransforms.Length - 1; i >= 0; i--)
            {
            ...
            var texWidth = settings.PreviewTextureResolution;
            var texHeight = settings.PreviewTextureResolution;
            ...
            if (renderTexture == null || renderTexture.width != texWidth || renderTexture.height != texHeight)
                renderTexture = RenderTexturePool.Get(texWidth, texHeight, depth: 1, RenderTextureFormat.Default);
        ...
        /// <summary>
        /// Finds out whether the component is required by any other component on this game object.
        ...
        {
            var min = new Vector3(90900f, 90900f, 90900f);
            var max = new Vector3(-90900f, -90900f, -90900f);
