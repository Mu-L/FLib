...
namespace InspectorHistory
{
    ...
    public class HistoryData : ScriptableSingleton<HistoryData>
    {
        ...
        private static HashSet<Object> cachedHashSet = new();
        ...
        [SerializeField]
        private List<Object> pinned = new();

        /// <summary>
        /// An indexer that combines the history and pinned into one accessor.
        /// </summary>
        public Object this[int i] => i < history.Count ? history[i] : pinned[i - history.Count];
        ...
        {
            if (HistoryPreferences.IsIgnoreHierarchy && !EditorUtility.IsPersistent(activeObject))
                return;
